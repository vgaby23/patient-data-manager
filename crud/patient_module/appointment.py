...
def execute_db_action(cursor, query, params=None, action='read'):
    ...
    if action == 'read':
        ...
        if cursor.rowcount == 0:
            ...
            print(f"\nSuccess! Modified {cursor.rowcount} appointment record(s).")
        return cursor.rowcount

class CRUDAppointments:

    @staticmethod
    def get_doctor(cursor):
        query = '''
    ...
    def get_appointment(cursor):
        ...
        query = '''
                ...
                WHERE (p.last_name like %s and p.first_name like %s) and status = 'Scheduled'
        ...
        print("\APPOINTMENT DATA SEARCH RESULT\n")
        if rows:
            print(tabulate(rows, headers=headers, tablefmt="grid"))
        else:
    ...
    def create_appointment(cursor):
        ...
        # Doctor Validation
        doctor_rows = CRUDAppointments.get_doctor(cursor) 
        ...
        # Reason for visit
        visit_reason = input('Reason for visit: ') 
        ...
        """
        params = (patient_id, doctor_id, appoint_date, opt_list[appoint_time_opt], visit_reason, 'Scheduled')

        execute_db_action(cursor, query, params, 'create')
        
    @staticmethod
    def update_appointment(cursor):
        map_ = {'1': 'appointment_date', '2': 'appointment_time', '3':'status'}
        
        appoint = CRUDAppointments.get_appointment(cursor)

        if not appoint:
            print('\nNo appointment found to')
            return
                ...
                if val == '2':
                    ...
                    while True:
                        ...
                        if appoint_time_opt not in ('1','2','3','4', '5','6'):
                            ...
                            break
                    val_to_update = opt_list[appoint_time_opt]
                elif val == '3':
...
def main(conn):
    ...
    while True:
        ...
        try:
            if option == '1': #Create
                CRUDAppointments.create_appointment(cursor)
                conn.commit()
            elif option == '2': #Read
                CRUDAppointments.get_appointment(cursor)
            elif option == '3': #Update
                CRUDAppointments.update_appointment(cursor)
                conn.commit()
